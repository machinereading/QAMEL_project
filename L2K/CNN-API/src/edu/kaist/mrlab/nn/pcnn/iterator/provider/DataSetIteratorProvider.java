...
public class DataSetIteratorProvider {
	...
	public DataSetIterator getDataSetIterator(boolean isTraining, WordVectors wordVectors,
		...
		for (OntoProperty p : OntoProperty.values()) {
			...
			if (isTraining) {
				path = "data/ds/train_test_" + GlobalVariables.option + "/" + property + "_train";
			} else {
				path = "data/ds/train_test_" + GlobalVariables.option + "/" + property + "_test";
			}
			...
			while ((input = br.readLine()) != null) {
				...
				if (isTraining == false) {
					if (property == "spouse") {
						...
						System.out.println(stcArr.length);
					}
				}
				*/
...
//		String path = "/test/wjd1004109/PL-Web-Demo/prepro/data/wiki_ex_PL4_parsed.txt";
		String path = "data/test/ko_test_parsed.txt";
//		String path = "data/test/wiki_ex_PL4_parsed.txt";
