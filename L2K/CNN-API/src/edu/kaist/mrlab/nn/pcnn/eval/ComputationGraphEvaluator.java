...
public class ComputationGraphEvaluator {
	...
	public List<String> singleEvaluate(DataSetIterator iterator) throws Exception {
		...
		Evaluation e = new Evaluation(labelsList, 1);
		while (iterator.hasNext()) {
			...
			for (int i = 0; i < guesses.rows(); i++) {
				...
				String plainSentence = sentence.replace("/VerbPrefix", "").replace("/Entity", "").replace("/Noun", "")
						.replace("/Punctuation", "").replace("/Suffix", "").replace("/Josa", "").replace("/Verb", "")
						.replace("/Number", "").replace("/Foreign", "").replace("/Alpha", "").replace("/ProperNoun", "")
						.replace("/Conjunction", "").replace("/Adjective", "").replace("/Adverb", "")
						.replace("/Exclamation", "").replace("/Determiner", "");

				tripleList.add(new TripleUnit(sbj, predicted, obj, predictedScore, plainSentence, answer));
			}
		...
		for (TripleUnit tu : tripleList) {
			...
			result.add(tu.getSbj() + "\t" + tu.getPredicted() + "\t" + tu.getObj() + "\t.\t" + tu.getScore() + "\t"
					+ tu.getSentence());
	...
	public Evaluation evaluate(DataSetIterator iterator, List<String> labelsList, int topN) throws Exception {
		...
		while (iterator.hasNext()) {
			...
			for (int i = 0; i < guesses.rows(); i++) {
				...
				// String originalStc = Testor.originalSentenceStore.getSentence(oriStcCount);
				// originalStc = originalStc.replaceAll("<< ", "");
				// originalStc = originalStc.replaceAll(" >>", "");
				String sentence = Testor.sentenceStore.getSentence(i);
				int sbjPos = Testor.positionStore.getSbjPos().get(i);
				int objPos = Testor.positionStore.getObjPos().get(i);
				...
				// System.out.println(sbj + "\t" + predicted + "\t" + obj + "\t" + guessScore +
				// "\t" + "\n");
				// System.out.println("com. graph : " + originalStc + "\t" + sentence);

				String origSentence = Testor.originalSentenceStore.getSentence(0);
				Testor.originalSentenceStore.remove(0);

				// String plainSentence = sentence.replace("/VerbPrefix", "").replace("/Entity",
				// "").replace("/Noun", "")
				// .replace("/Punctuation", "").replace("/Suffix", "").replace("/Josa",
				// "").replace("/Verb", "")
				// .replace("/Number", "").replace("/Foreign", "").replace("/Alpha",
				// "").replace("/ProperNoun", "")
				// .replace("/Conjunction", "").replace("/Adjective", "").replace("/Adverb", "")
				// .replace("/Exclamation", "").replace("/Determiner", "");
		...
		for (TripleUnit tu : tripleList) {
			...
			// if (tu.getLabel().equals(tu.getPredicted())) {
			// right++;
			// } else {
			// wrong++;
			// }

			// bw.write(tu.getLabel() + "\t" + tu.getPredicted() + "\t" + tu.getScore() +
			// "\n");
			bw.write(tu.getSbj() + "\t" + tu.getPredicted() + "\t" + tu.getObj() + "\t.\t" + tu.getScore() + "\t"
					+ tu.getSentence() + "\n");
			// bw.write(tu.getSbj() + "\t" + tu.getObj() + "\t" + tu.getPredicted() + "\t" +
			// tu.getLabel() + "\t"
			// + tu.getScore() + "\t" + tu.getSentence() + "\n");
			// bw.write(tu.getSbj() + "\t" + tu.getObj() + "\t" + tu.getLabel() + "\t" +
			// tu.getPredicted() + "\t"
			// + tu.getScore() + "\t" + tu.getSentence() +"\n");
