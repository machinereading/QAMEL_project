...
//	public static final String WORD_VECTORS_PATH = "data/embedding/ko_vec_100dim_1min_word_stem";
	...
	public static void main(String[] ar) throws Exception {
		...
		int truncateReviewsToLength = GlobalVariables.truncateSentencesToLength; // Truncate reviews with length (# words) greater than this
		int numOfRelation = OntoProperty.values().length;

		int cnnLayerFeatureMaps = GlobalVariables.cnnLayerFeatureMaps; // Number of feature maps / channels / depth for each CNN layer
		...
		DataSetIterator trainIter = dsip.getDataSetIterator(true, wordVectors, positionVectors, POSVectors, batchSize, truncateReviewsToLength, rng, t);
		DataSetIterator testIter = dsip.getDataSetIterator(false, wordVectors, positionVectors, POSVectors, batchSize, truncateReviewsToLength, rng, t);
		long end = System.currentTimeMillis();
		
		System.out.println("Embedding vectors and T/T data set loading completed. Takes " + (end-start)/1000 + "sec.");
		...
		// to save the model
		File locationToSave = new File("data/model/CNN_RELU_ADAM_" + GlobalVariables.option + ".zip");
