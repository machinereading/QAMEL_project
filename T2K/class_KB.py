class KB:
	...
	def load_schema(self):
		with open('data/KB/user-schema.json') as i_file:
			...
			user_schema = json.loads(i_file.read())

			# --

			for p, p_types, domains, ranges in user_schema['domain/range']:
			...
			for entity, types in user_schema['entity-type']:
				for type_ in types:
					try:
						self.entity2type[entity].add(self.utility.norm_name(self.utility.uri2name(type_)))

					except KeyError:
						self.entity2type[entity] = set([self.utility.norm_name(self.utility.uri2name(type_))])
		# --
	...
	def check_domain_range(self, KB_s, KB_p, KB_o):
		...
		print('(s, p, o)', KB_s, KB_p, KB_o)
		print('p_domain', p_domain)
		print('p_range', p_range)
		print('s_types', s_types)
		print('o_types', o_types)
